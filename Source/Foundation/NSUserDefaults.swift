...
extension UserDefaults {
    ...
    public func rex_valueForKey(_ key: String) -> SignalProducer<Any?, NoError> {
        let center = NotificationCenter.default
        let initial = object(forKey: key)

        let changesSignal = center.reactive.notifications(forName: UserDefaults.didChangeNotification)
            .map { _ in
